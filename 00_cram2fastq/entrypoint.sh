...
VERSION="1.1"
# Entrypoint script for docker container achalneupane/cram2bam:${VERSION}, modified from vifehe/revertbam
...
if [ -z "${CRAMFILE}" ]; then
    echo "ERROR, must provide cram or bam file in variable \${CRAMFILE}"
    quit "Input File"
fi
if [ ! -r "${CRAMFILE}" ]; then
